  ...
  def apply(t: Tree): String = {
    ...
    s += (t match {
      ...
      case MainDecl(obj, parent, vars, exprs) =>
        "MainDecl(" + apply(obj) + "," + handleList(vars) + "," + handleList(
          exprs
        )
      case ClassDecl(id, parent, vars, methods) =>
        "ClassDecl(" + apply(id) + "," + handleList(vars) + "," + handleList(
          methods
        )
      ...
      case MethodDecl(overrides, retType, id, args, vars, exprs, retExpr) =>
        "MethodDecl(" + overrides + "," + apply(retType) + "," + methodId(id) + "," + handleList(
          args
        ) + "," + handleList(vars) + "," + handleList(exprs) + "," + apply(
          retExpr
        )
      ...
      case MethodCall(obj, meth, args) =>
        "MethodCall(" + apply(obj) + "," + methodId(meth) + "," + handleList(
          args
        )
      case IntLit(value) =>
        doIndent = false
    ...
    s += (if (t.isInstanceOf[ExprTree]) {
            ":" + t.asInstanceOf[ExprTree].getTypeStr
          } else {
            ""
          })
    s
